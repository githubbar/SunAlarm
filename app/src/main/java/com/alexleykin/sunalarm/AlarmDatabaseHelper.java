    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE alarms (" +
                   ...
                   "sunrise_duration INTEGER, " +
                    "sunrise_turnoff INTEGER);");

        // insert default alarms
        String insertMe = "INSERT INTO alarms " +
                "(hour, minutes, daysofweek, alarmtime, enabled, vibrate, " +
                " message, alert, sunrise_duration, sunrise_turnoff) VALUES ";
        db.execSQL(insertMe + "(8, 30, 31, 0, 0, 1, '', '', 5, 30);");
        db.execSQL(insertMe + "(9, 00, 96, 0, 0, 1, '', '', 5, 30);");
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion,
            int currentVersion) {
        // NOTE: this is where we add ne database columns
        if (Log.LOGV) Log.v(
