...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    private void handleIntent(Context context, Intent intent) {
        ...
        // Play the alarm alert and vibrate the device. Starts AlarmKlaxon service
        // TODO: seems like we need to play the lightbulb stuff here. Which class and method does this call?
        Intent playAlarm = new Intent(Alarms.ALARM_ALERT_ACTION);
        playAlarm.putExtra(Alarms.ALARM_INTENT_EXTRA, alarm);
        Intent explicit = this.createExplicitFromImplicitIntent(context, playAlarm);
        ...
        builder.setOngoing(Notification.FLAG_ONGOING_EVENT != 0);

        // Embed the full-screen UI here. The notification manager will
        // take care of displaying it if it's OK to do so.
        Intent alarmAlert = new Intent(context, c);
        alarmAlert.putExtra(Alarms.ALARM_INTENT_EXTRA, alarm);
        alarmAlert.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
                | Intent.FLAG_ACTIVITY_NO_USER_ACTION);
        builder.setFullScreenIntent(PendingIntent.getActivity(context, alarm.id, alarmAlert, 0), true);
    ...
    private void updateNotification(Context context, Alarm alarm, int timeout) {
        ...
        builder.setAutoCancel(Notification.FLAG_AUTO_CANCEL != 0);
        Notification n  = builder.build();
